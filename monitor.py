"""
NOL 票务监控 — Bark 通知版
依赖: pip install requests beautifulsoup4 cloudscraper
"""
...
def fetch_html():
    ...
    resp = scraper.get(NOL_URL, timeout=30)
    resp.raise_for_status()
    return resp.text

# ── 解析演出列表（先试 Next.js JSON，再试 HTML）─────────────────────────────
def extract_next_data(html):
    ...
    def walk(obj, depth=0):
        ...
        if (isinstance(obj, list) and len(obj) >= 3
                and all(isinstance(i, dict) for i in obj)):
            if any(k in obj[0] for k in ["title","name","productName","goodsName","performName"]):
                return obj
...
def notify(title, body):
    ...
    try:
        r = requests.post("https://api.day.app/push", json={
            ...
            "group": "NOL监控", "url": "https://world.nol.com/en/ticket",
            "sound": "alarm",
        }, timeout=10)
        d = r.json()
        log("✅ Bark 通知已发送" if d.get("code") == 200 else f"❌ Bark: {d.get('message')}")
...
def main():
    ...
    if notified:
        for c in notified:
            log(f"🔔 {c['title']}: {c['body'].splitlines()[0]}")
            notify(c["title"], c["body"])
